...
const { ciCollectModulePackageDependencies, ciResolveModuleGraph } =
  ciModuleHelpers;
...
async function ciValidateModules() {
  ...
  try {
    entries = await readdir(ciModulesDirectory, {
      withFileTypes: true,
    });
  } catch (error) {
    if (
      ...
      console.log(
        `✅ No application modules directory exists at ${ciModulesDirectory}; nothing to validate.`,
      );
      return;
    }
