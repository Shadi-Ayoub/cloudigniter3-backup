...
export const ciGetServerStatus = async (
  ...
  const systemStatus: CiSystemStatus = {
    overall: "success",
...
export function amplifyOutputsStatus(
  amplifyConfig: CiAmplifyOutputs,
): CiSystemStatusItem {
  if (amplifyConfig) {
    return {
      status: "success",
      message:
        "CloudIgniter loaded the Amplify backend configurations successfully.",
...
export async function amplifySchemaStatus(
  isOk: boolean,
): Promise<CiSystemStatusItem> {
  ...
  if (isOk) {
    return {
      status: "success",
      message:
        "CloudIgniter could reference Amplify backend Schema type successfully.",
      key: "amplifySchema",
...
const getOverallStatus = (
  items: CiSystemStatus,
): CiSystemStatusItem["status"] => {
