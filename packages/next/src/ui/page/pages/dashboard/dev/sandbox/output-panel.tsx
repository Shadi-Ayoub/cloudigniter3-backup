...
import { type Response, type SandboxMethodDefinition } from '@CI/types';
...
const OutputPanel = ({ getInput, selectedMethod, setLoading }: OutputPanelProps) => {
  ...
  const runMethod = async () => {
    try {
      ...
      if (input === '') {
        result = await (selectedMethod.callback as () => Promise<Response>)();
      } else {
        result = await (selectedMethod.callback as (input: string) => Promise<Response>)(input);
      }
      ...
      <div className='relative min-h-[200px] rounded border border-gray-300 bg-gray-50 p-4 shadow dark:border-gray-700 dark:bg-gray-800'>
        <div className='flex items-center justify-between rounded bg-green-500 p-2 text-white dark:bg-green-700'>
          ...
          <div className='flex items-center gap-2'>
            <button onClick={runMethod} className='hover:text-gray-300 dark:hover:text-gray-400'>
              <FaPlay />
            </button>
            <button onClick={clearOutput} className='hover:text-gray-300 dark:hover:text-gray-400'>
              <FaTrash />
            </button>
          </div>
        </div>
