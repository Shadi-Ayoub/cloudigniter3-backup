...
import { usePageLoader } from '@CI/store';

import type { CiRequest, Response, SeederInput, SeederResponseBody, SeederErrorBody, SeederItemKey } from '@CI/types';

type LogLine = { ts: string; text: string; tone?: 'info' | 'success' | 'warning' | 'error' };

export function SeederPage() {
  const { isLoading, setLoading } = usePageLoader();
  const locale = useLocale();
  const messages = useMessages();
  const [successMsg, setSuccessMsg] = useState<string | null>(null);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  const [checked, setChecked] = useState<Record<SeederItemKey, boolean>>({
    users: false,
    tenants: false,
    orgUnits: false,
  });
  ...
  const selectedItems = useMemo(() => (Object.keys(checked) as SeederItemKey[]).filter((k) => checked[k]), [checked]);
  ...
  function isSeederOkBody(x: unknown): x is SeederResponseBody {
    return (
      !!x &&
      typeof x === 'object' &&
      'action' in x &&
  ...
  async function run(action: 'seed' | 'clear') {
    ...
    const request: CiRequest<SeederInput> = {
      input: { action, items: selectedItems },
      envMode,
    };

    setIsBusy(true);
    append(`${action.toUpperCase()} started for: ${selectedItems.join(', ')}`, 'info');

    try {
      const result = await call<SeederInput, Response<SeederResponseBody, SeederErrorBody>>(
        '/dashboard/dev/seeder/seed',
        request
      );
      ...
      if (!ok) {
        append(`Status: ${requestHttpStatusCode}`, requestHttpStatusCode >= 400 ? 'error' : 'success');
        append(`Status: ${result.message}`, requestHttpStatusCode >= 400 ? 'error' : 'success');
      ...
      if (!isCiResponse<SeederResponseBody, SeederErrorBody>(respUnknown)) {
      ...
      if (statusCode >= 400) {
        ...
        const errBody = respUnknown.body as SeederErrorBody;
        const errMsg =
      ...
      if (!isSeederOkBody(respUnknown.body)) {
        const errMsg = `Unexpected OK body shape: ${safeToString(respUnknown.body)}`;
        append(errMsg, 'error');
        setErrorMsg(errMsg);
        notify('error', errMsg);
        return;
      ...
      for (const r of results) {
        append(
          `${r.item}: ${r.ok ? 'OK' : 'FAILED'}${typeof r.count === 'number' ? ` (count=${r.count})` : ''}${
            r.message ? ` — ${r.message}` : ''
          }`,
          r.ok ? 'success' : 'error'
        );
        ...
        if (!r.ok && r.error) {
          append(`${r.item}: error detail: ${safeToString(r.error)}`, 'error');
        }
      }
          ...
          <div className='grid min-h-0 flex-1 grid-cols-12 gap-3'>
            ...
            <div className='bg-background col-span-12 flex min-h-0 flex-col rounded-xl border md:col-span-4 lg:col-span-3'>
              <div className='shrink-0 border-b p-3'>
                ...
                <div className='text-muted-foreground text-xs'>Check what you want to seed/clear</div>
              </div>
            ...
            <div className='bg-background col-span-12 flex min-h-0 flex-col rounded-xl border md:col-span-8 lg:col-span-9'>
              <div className='shrink-0 border-b p-3'>
                ...
                <div className='text-muted-foreground text-xs'>Latest events first</div>
              </div>
                          ...
                          {log.map((l, idx) => (
                            <li key={`${l.ts}-${idx}`} className='leading-none'>
                              <div className='flex min-w-0 items-center gap-2'>
                                ...
                                <span className='shrink-0 text-xs leading-none opacity-50'>$</span>

                                {/* Tone column: dynamic width in "ch" so message starts at same column */}
                                <span
                                  className={`shrink-0 text-xs leading-none tracking-wide uppercase tabular-nums ${toneClass(l.tone)}`}
                                ...
                                <span className='min-w-0 text-xs leading-none break-words opacity-90'>{l.text}</span>
                              </div>
                            </li>
                          ))}
                        </ul>
