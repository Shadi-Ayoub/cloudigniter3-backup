...
import type { AuthenticatorConfig } from '@CI/types';
...
export function useShowLoaderWhenAuthenticatorDisappears(
  ...
  ciConfig: AuthenticatorConfig
) {
  const { minHeightPx = 16, debounceMs = 0, initialMountSuppressMs = 1200, minVisibleStableMs = 300 } = opts;

  /**
   * Zustand action that toggles the global page loader.
   * Assumption based on your usage: setLoading(loading: boolean, text?: string)
   */
  const setLoading = usePageLoader((s) => s.setLoading);
  ...
  React.useEffect(() => {
    ...
    const check = () => {
      ...
      const signingInText = ciConfig.custom?.signinSpinnereText ?? 'Signing you in. Please wait.';

      const sinceMountMs = now - mountedAtRef.current;
      const visibleStableMs = firstVisibleAtRef.current == null ? 0 : now - firstVisibleAtRef.current;
      ...
      const shouldUseSignInText =
        hasEverBeenVisibleRef.current && sinceMountMs > initialMountSuppressMs && visibleStableMs > minVisibleStableMs;

      const nextLabel: 'initial' | 'signin' = shouldUseSignInText ? 'signin' : 'initial';
  ...
  }, [containerRef, minHeightPx, debounceMs, initialMountSuppressMs, minVisibleStableMs, setLoading, ciConfig]);
}
