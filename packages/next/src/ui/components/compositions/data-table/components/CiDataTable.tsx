...
import { buildColumnsWithActions } from "../lib/build-columns-with-actions";
import { CiRowActionsMenu } from "./CiRowActionsMenu";
import type {
  CiCursorPage,
  CiCursorQuery,
  CiDataMode,
...
export function CiDataTable<TData, TValue>({
  ...
  const effectiveMode: Exclude<CiDataMode, "auto"> =
    mode === "auto"
      ? source
        ? "hybrid"
        : "client"
  ...
  const [page, setPage] = useState<CiCursorPage<TData>>({ rows: [] });
  const [isLoading, setIsLoading] = useState(false);
  const [errMsg, setErrMsg] = useState<string | null>(null);

  // Hybrid cache
  const cacheRef = useRef<CiPageCache<TData>>(new Map());

  const columnsWithActions = useMemo<ColumnDef<TData, TValue>[]>(() => {
    return buildColumnsWithActions<TData, TValue>({
      columns,
      rowActions,
      actionsHeader: "Actions",
      renderActionsCell: (ctx, actions) => (
        <div className="flex justify-start rtl:justify-end">
          <CiRowActionsMenu row={ctx.row.original} actions={actions} />
        </div>
      ),
    });
  }, [columns, rowActions]);
  ...
  const prefetchNext = useCallback(
    async (base: CiCursorQuery, nextCursor: string) => {
      if (!source) return;
      const q: CiCursorQuery = { ...base, cursor: nextCursor };
