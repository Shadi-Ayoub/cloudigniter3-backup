...
export interface SideTabsListProps {
  loaded: boolean;
  defaultTab?: BeaconTabValue; // 'status' | 'config' | 'tools'
  className?: string;

  /** Optional content for each tab (renders inside the right pane) */
  ...
  return (
    <aside className={cn('border-r p-2 data-[side=right]:border-r-0 data-[side=right]:border-l', className)}>
      <Tabs defaultValue={defaultTab} orientation='vertical' className='flex w-full flex-row items-start gap-3'>
        ...
        {/* Right: content panel */}
        <div className='flex min-h-40 w-full max-w-full items-stretch justify-stretch rounded-md border'>
          <div className='flex-1 p-3'>
            <TabsContent value='status' className='m-0 flex h-full items-start justify-start'>
              {statusContent ?? <div className='text-muted-foreground text-sm'>No status content.</div>}
            </TabsContent>

            <TabsContent value='config' className='m-0 flex h-full items-start justify-start'>
              {configContent ?? <div className='text-muted-foreground text-sm'>No config content.</div>}
            </TabsContent>

            <TabsContent value='tools' className='m-0 flex h-full items-start justify-start'>
