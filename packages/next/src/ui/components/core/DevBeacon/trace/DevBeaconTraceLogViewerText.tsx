...
import { useMonacoTheme } from '../../../compositions/smart-form/hooks/useCiMonacoTheme';

export interface DevBeaconTraceLogViewerTextProps {
...
export function DevBeaconTraceLogViewerText({
  ...
  autoScroll = true,
  wordWrap = 'off',
}: DevBeaconTraceLogViewerTextProps) {
  ...
  const decoRef = React.useRef<monacoNS.editor.IEditorDecorationsCollection | null>(null);
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      const body = await safeFetchText(`${endpoint}${query}`, { signal: ctrl.signal });
      setText(body);
    ...
    <div className='w-full space-y-3 p-3 text-sm'>
      ...
      <div className='flex items-center gap-2'>
        ...
        <div className='ml-auto flex items-center gap-2'>
          {updatedAt ? (
            <span className='text-muted-foreground text-xs'>Updated {new Date(updatedAt).toLocaleTimeString()}</span>
          ) : null}
