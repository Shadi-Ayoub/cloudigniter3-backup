...
export function DevBeaconTraceTab({
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      const data = await safeFetchJson(`${endpoint}${query}`, { signal: ctrl.signal });
    ...
    <div className='space-y-3 p-3 text-sm'>
      <div className='flex items-center gap-2'>
        ...
        <div className='ml-auto flex items-center gap-2'>
          {lastUpdated ? (
            <span className='text-muted-foreground text-xs'>Updated {new Date(lastUpdated).toLocaleTimeString()}</span>
          ) : null}
            ...
            {rows.length === 0 ? (
              ...
              rows.map((e) => (
                <tr key={e.seq} className='[&>td]:px-2 [&>td]:py-1 [&>td]:align-top'>
                  ...
                  <td className='whitespace-nowrap'>{new Date(e.ts).toLocaleTimeString()}</td>
                  ...
                  <td className='whitespace-nowrap'>{e.requestId?.slice(0, 8)}</td>
                  <td className='whitespace-nowrap'>{e.traceId?.slice(0, 8)}</td>
                  <td>
...
export function devBeaconGetTraceTab(overrides?: TraceTabProps) {
  return {
    id: 'trace',
    label: 'Trace',
    icon: ListTree,
    content: <DevBeaconTraceTab {...overrides} />,
  };
}
