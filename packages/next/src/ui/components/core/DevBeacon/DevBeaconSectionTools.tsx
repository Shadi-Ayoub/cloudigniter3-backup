...
export function DevBeaconSectionTools({ onMarkLoaded }: SectionToolsProps) {
  React.useEffect(() => {
  ...
  async function startWave(custom?: string) {
    ...
    try {
      ...
      setResult({ ok: true, msg: `Started wave${body.label ? `: ${body.label}` : ''}` });
      ...
      <div className='grid grid-cols-1 gap-3 sm:grid-cols-2'>
        ...
        <div className='rounded-lg border p-3 sm:col-span-2'>
          ...
          <p className='text-muted-foreground text-sm'>
            Inserts a separator in the trace file and Dev Beacon timeline. Useful before/after major actions.
          </p>
          ...
          <div className='mt-3 grid grid-cols-1 items-end gap-2 sm:grid-cols-[1fr_auto_auto]'>
            ...
            />
            <Button size='sm' variant='secondary' disabled={busy} onClick={() => startWave()} className='sm:ml-2'>
              {busy ? 'Starting…' : 'Start Wave'}
            </Button>
            <Button size='sm' variant='ghost' disabled={busy} onClick={() => startWave(tsLabel())} className='sm:ml-2'>
              Start w/ Timestamp
            </Button>
          </div>

          {result && (
            <div className={`mt-2 text-xs ${result.ok ? 'text-emerald-600' : 'text-red-600'}`}>{result.msg}</div>
          )}
        </div>
      </div>
    </div>
