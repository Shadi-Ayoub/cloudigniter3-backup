...
import {
  ...
  SCALE_PREFIXES,
  STATES,
} from "./colors"; // Color related constants
...
function makeInitialSnapshot(): Snapshot {
  ...
  const emptyBy = (keys: readonly string[]) =>
...
function readSnapshot(): Snapshot {
  ...
  const animations = ANIMATE_VAR_NAMES.map((name) => ({
    name,
    value: read(name),
  }));
      ...
      <div className="mx-auto max-w-7xl space-y-10">
        ...
        <Section title="Base Colors (Dark palette names)">
          <Card>
            ...
            <p className="mt-3 text-sm opacity-80">
              These are *named* dark tokens. Your app may still use the light
              tokens at runtime unless a `.dark` class flips usage.
            </p>
          </Card>
        </Section>
        ...
        <Section title="State Colors (Light / Dark)">
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {STATES.map((key) => {
              ...
              const lightBgCaption = mounted
                ? readVar(lightBgVar) || `var(${lightBgVar})`
                : `var(${lightBgVar})`;
              const lightFgCaption = mounted
                ? readVar(lightFgVar) || `var(${lightFgVar})`
                : `var(${lightFgVar})`;
              const darkBgCaption = mounted
                ? readVar(darkBgVar) || `var(${darkBgVar})`
                : `var(${darkBgVar})`;
              const darkFgCaption = mounted
                ? readVar(darkFgVar) || `var(${darkFgVar})`
                : `var(${darkFgVar})`;
              ...
              return (
                <Card key={key}>
                  ...
                  <div
                    ...
                    style={{
                      background: `var(${lightBgVar})`,
                      color: `var(${lightFgVar})`,
                    }}
        ...
        <Section title="Motion / Animations">
          ...
          <p className="mt-2 text-xs opacity-70">
            Note: if your gradient animation isn't running, double-check the
            spelling of the keyframes name (gradient vs gradiant).
          </p>
        </Section>
      </div>
    </>
