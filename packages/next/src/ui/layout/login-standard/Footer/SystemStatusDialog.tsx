...
import { Dialog, DialogTrigger, DialogContent, DialogHeader, DialogTitle } from '@CI/ui/components/shadcn/dialog';
import { Button } from '@CI/ui/components/shadcn/button';
import { ScrollArea } from '@CI/ui/components/shadcn/scroll-area';
import type { SystemStatus, SystemStatusCheckList, SystemStatusItem } from '@CI/types';

// type ViewMode = 'amplifyConfig' | 'systemSettings' | null;

const getStatusDot = (status: SystemStatusItem['status']) => (status === 'success' ? 'bg-green-500' : 'bg-red-500');

const getOverallStatusText = (status: SystemStatusItem['status']) =>
...
export const SystemStatusDialog = ({ checkList }: SystemStatusDialogInterface) => {
  const status = checkList.status as SystemStatus;
  ...
  const entries: [Exclude<keyof SystemStatus, 'overall'>, SystemStatusItem][] = Object.keys(status)
    .filter((key): key is Exclude<keyof SystemStatus, 'overall'> => key !== 'overall')
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <button className='flex cursor-pointer items-center gap-2' aria-label='System Status'>
          <div className={`h-3 w-3 rounded-full ${getStatusDot(overallStatus)}`} />
          ...
          <div className='space-y-4'>
            {entries.map(([key, item]) => (
              <div key={key} className='flex items-start gap-3'>
                <div className={`mt-1 h-3 w-3 shrink-0 rounded-full ${getStatusDot(item.status)}`} />
                ...
                {item.status === 'success' && item.configJson && (
                  <Button size='sm' variant='outline' onClick={() => setViewingJson(item.key as string)}>
                    View
                  </Button>
                )}
              </div>
          ...
          <div className='space-y-2'>
            <div className='flex items-center justify-between'>
              <DialogTitle className='text-sm'>
                {viewingJson === 'amplifyConfig' ? 'Amplify Configuration JSON' : 'System Settings JSON'}
              </DialogTitle>
              <Button size='sm' variant='ghost' onClick={() => setViewingJson(null)}>
                Back
              </Button>
            </div>
            ...
            <ScrollArea className='h-64 rounded-md border p-1'>
              <pre className='text-xs break-words whitespace-pre-wrap'>
                {viewingJson === 'amplifyConfig'
                  ? amplifyConfigItem?.status === 'success' && amplifyConfigItem.configJson
