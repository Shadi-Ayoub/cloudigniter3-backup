...
export function CiDevBeaconProvidersStatusSegment({
  context,
}: CiDevBeaconProvidersStatusProps) {
  const providers = context.config.appCoreConfig.providers ?? [];
  const awsProvider = context.config.appCoreConfig.providers?.aws;

  const awsProviderStatusAmplifyOutputsOk =
    context.status?.providers?.aws?.amplifyOutputs?.check === true;

  const awsProviderStatusSchemasOk =
    context.status?.providers?.aws?.schema?.check === true;
  ...
  return (
    <>
      ...
      {awsProvider ? (
        <CiDevBeaconCardRowGrid columns={2} boxed={true} cellPadding="compact">
          ...
          <CiDevBeaconCardRow
            ...
            tooltip={
              <>
                Indicates whether the application's data schema was loaded and
