    ...
    switch (options.format) {
      ...
      case "JSON":
        ...
        if (options.caption) {
          console.log(
            `%c${
              options.caption.charAt(0).toUpperCase() + options.caption.slice(1)
            } JSON:`,
            `font-weight: normal; color: ${color};`,
          );
        }
        console.log(
          `%c${JSON.stringify(message, null, 2)}`,
          `font-weight: normal; color: ${color};`,
        );
        break;
      default: // TEXT
...
export function printTable(
  ...
  if (Array.isArray(data) && data.every((item) => typeof item === "object")) {
    ...
    if (data.length !== 0) {
      console.log(
        `%c${
          tableCaption.charAt(0).toUpperCase() + tableCaption.slice(1)
        } Table:`,
