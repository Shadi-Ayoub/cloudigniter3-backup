...
// const external = ["class-variance-authority/types"]; // To silence warnings during build types bundling!
const external = [
  "@cloudigniter/core",
...
const aliasPlugin = alias({
  entries: [
    ...
    {
      find: "@ci-aws/server/backend",
      replacement: path.resolve(
        projectRoot,
        "dist/.types/server/backend/index.d.ts",
      ),
    },
    {
      find: "@ci-aws/lib",
...
const config = [
  ...
  {
    input: "dist/.types/server/backend/index.d.ts",
    output: {
      file: "dist/server/backend/index.d.ts",
      format: "es",
    },
    external,
    plugins: [aliasPlugin, dts()],
