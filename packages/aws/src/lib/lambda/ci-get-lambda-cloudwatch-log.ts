...
import type { CiLambdaReportLog } from "@ci-aws/types";

type CiGetLambdaCloudwatchLogBody = CiLambdaReportLog | null;
...
export async function ciGetLambdaCloudwatchLog(
  ...
  try {
    ...
    if (logStreams.length === 0) {
      return ciOk200<CiGetLambdaCloudwatchLogBody>(null);
    }
    ...
    for (const stream of logStreams) {
      ...
      if (parsed) {
        return ciOk200<CiGetLambdaCloudwatchLogBody>(parsed);
      }
    }
    ...
    return ciError500<CiGetLambdaCloudwatchLogBody>(
