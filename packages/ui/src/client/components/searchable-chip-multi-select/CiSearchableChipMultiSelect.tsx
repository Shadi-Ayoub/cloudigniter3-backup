...
export function CiSearchableChipMultiSelect({
  ...
  const open =
    !disabled && focused && (showAllOptions || query.trim().length > 0);
      ...
      {selectedItems.length > 0 ? (
        ...
        >
          {sortedSelectedItems.map((item) => (
            <span
              key={item.id}
              className="inline-flex min-h-8 max-w-full items-center gap-1 rounded-full border bg-background py-1 pe-1 ps-3 text-sm shadow-xs"
            >
              <span className="truncate">{item.label}</span>
              <button
                type="button"
                disabled={disabled}
                className="relative inline-flex size-9 shrink-0 cursor-pointer items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-destructive/10 hover:text-destructive focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none after:absolute after:-inset-1 disabled:cursor-not-allowed disabled:opacity-50"
      ...
      <div className="relative">
        {showAllOptions ? (
          <button
            ...
            className="flex min-h-11 w-full cursor-pointer items-center justify-between rounded-lg border bg-background px-3 text-start text-sm shadow-xs transition-colors hover:bg-accent focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50"
            onClick={() => {
            ...
            <Input
              ...
              onKeyDown={(event) => {
                if (event.key === "ArrowDown" && matches.length > 0) {
                  ...
                  const lastItem =
                    sortedSelectedItems[sortedSelectedItems.length - 1];
          ...
          {matches.length > 0 ? (
            matches.map((option, index) => (
              <button
                ...
                className="flex min-h-11 w-full cursor-pointer flex-col items-start rounded-md px-3 py-2 text-start text-sm transition-colors hover:bg-accent hover:text-accent-foreground data-[active=true]:bg-accent data-[active=true]:text-accent-foreground"
