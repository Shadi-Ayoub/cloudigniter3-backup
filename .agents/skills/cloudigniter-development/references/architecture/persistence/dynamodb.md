...
Shared Org Unit trees use one canonical node plus strongly consistent tenant/path attachment items. This avoids duplicating a shared department while retaining direct tenant routing lookups. Management uses the existing System-table GSI; no new table or index is required.
