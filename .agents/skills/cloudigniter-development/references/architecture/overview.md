...
Use this reference for broad architecture, dependency direction, and the template-versus-package boundary; use [page rendering](rendering/page-rendering.md) for the root-layout, route-layout, page, bootstrap, and provider hierarchy.
...
Application-facing generators may update only their registered resource folders and explicitly generated registries within these seams. They must reject core/manual/generated key and filesystem-path collisions before mutation. Read the [template core/custom boundary](packages/template-core-custom-boundary.md) for the complete ownership, rollback, and upgrade contract.

For focused follow-up, use [package ownership](packages/ownership.md), [public API and runtime boundaries](packages/public-api-and-runtime.md), [Tenant request lifecycle](tenants/overview.md), [resource architecture](resources/overview.md), or [EmberGuard](security/emberguard.md).
