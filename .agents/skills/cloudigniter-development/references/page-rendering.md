...
Bootstrap depends on the upstream proxy, request-context, and next-intl lifecycle described in [request-lifecycle.md](request-lifecycle.md). Do not add route or locale fallback resolution to a rendering wrapper to conceal missing bootstrap data.
...
- conditionally renders `CiDevBeacon` after checking environment and actor access;
- renders the root `CiDebugProbe`;
