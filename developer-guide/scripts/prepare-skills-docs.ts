...
function writeCategoryMetadata(directory: string, label: string, position?: number): void {
  fs.mkdirSync(directory, { recursive: true });
  const metadata = position === undefined ? { label, collapsed: true } : { label, position, collapsed: true };
  fs.writeFileSync(
        ...
        const position =
          entry.name === "cloudigniter-development"
            ...
            : entry.name === "cloudigniter-guide-authoring"
              ? -1
              : undefined;
        writeCategoryMetadata(skillDir, entry.name.replaceAll("-", " "), position);
        if (fs.existsSync(path.join(skillDir, "references"))) {
          writeCategoryMetadata(path.join(skillDir, "references"), "References");
        }
      }
    }
  }
