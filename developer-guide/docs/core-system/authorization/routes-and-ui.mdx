...
For the application registry, `src/kernel/server/routes.ts`, `src/custom/routes`, and route fields such as `protected`, start
with [Routes configuration](/docs/configuration/routes).
...
This is metadata. `@cloudigniter/emberguard` remains platform-neutral and does not automatically redirect or return an
