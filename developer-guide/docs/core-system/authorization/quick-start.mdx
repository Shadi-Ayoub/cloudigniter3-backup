...
Import authorization helpers from `@cloudigniter/core/lib` and types from `@cloudigniter/core/types`. These are
the public entry points for application authorization.
...
```ts
import { ciOrgUnitAccessScope } from "@cloudigniter/core/lib";
