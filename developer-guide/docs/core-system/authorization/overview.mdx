...
Use the public authorization helpers from `@cloudigniter/core/lib` and types from `@cloudigniter/core/types`.
CloudIgniter uses EmberGuard to evaluate roles, privileges, and scopes independently of the identity or storage
provider. Applications and provider adapters supply plain TypeScript data, and the engine produces a
