...
The implementation lives in `@cloudigniter/emberguard`. It does not import Next.js, AWS, Cognito, DynamoDB, or any
other provider SDK. Applications and provider adapters supply plain TypeScript data; the engine produces a
deterministic authorization decision. `@cloudigniter/core` re-exports the current API as a compatibility bridge.
