...
const request = {
  ...
  action: "approve",
  scope: ciOrgUnitAccessScope("tenant-123", "finance", ["company"]),
};
```
...
| `suspended-role`        | A matching scoped assignment points to a suspended role and no other scoped grant applies. |
...
```ts
import { ciAuthorize, ciCan } from "@cloudigniter/emberguard/lib";
