...
Authorization evaluates the resource **type** and request scope. If access depends on a record's owner,
...
:::note Privilege titles
Every complete privilege definition requires a non-empty `title`. When a persisted catalog contains a privilege
without a title, CloudIgniter recovers it from the matching configured privilege or derives a deterministic label
from the privilege ID. Review these labels and save the catalog to persist them.
