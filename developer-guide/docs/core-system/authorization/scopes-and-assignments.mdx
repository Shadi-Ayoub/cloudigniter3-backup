...
import { ciCreateRoleAssignmentsFromIdentityGroups } from "@cloudigniter/core/lib";
...
An assignment on `finance` with `descendants` contains this request. An assignment on `operations` does not.
...
import { ciAccessScopeContains } from "@cloudigniter/core/lib";
