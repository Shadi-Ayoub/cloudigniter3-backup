...
All saves and reads use this canonical address; records stored under other keys are not resolved automatically.
If you import an existing catalog, migrate its record to this address before enabling authorization. See
[Changing an existing key](/docs/library/dynamodb/table-key-convention#changing-an-existing-key) for migration considerations.
...
## Loading catalogs with missing titles {#legacy-privilege-title-compatibility}

When a stored definition omits a privilege’s `title`, CloudIgniter supplies it when loading the catalog. It first
recovers the title from a matching privilege in the configured catalog, then derives a human-readable label from the stable ID for
an unknown custom privilege. For example, `approve-invoices` becomes `Approve invoices`.

Loading these values does not write them to DynamoDB. Review the generated titles and save the catalog to persist
them. Explicitly blank titles and unrelated invalid catalog fields still fail validation and require correction.
...
authorizers must be recreated after a status-changing write. An omitted status is interpreted as `active`.

Resource-domain and individual-resource suspension use the same additive `status` and `statusChange` pattern in
the active definition. They reuse the existing optimistic `SetEmberguardDefinition` transaction, so the
catalog and role-counter projection change atomically without a separate lifecycle table, index, or
authorization-path query. A suspended domain remains the parent denial for every child resource; restoring one
resource does not bypass that domain state.
