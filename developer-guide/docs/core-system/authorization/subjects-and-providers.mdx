...
```ts
import { ciCreateAuthorizationSubject } from "@cloudigniter/core/lib";
import type {
  CiRoleAssignment,
  CiScopedPrivilege,
  CiUser,
...
const systemAssignments = ciCreateRoleAssignmentsFromIdentityGroups(
  ...
  "exact",
  {
    roleMap: {
      ...
      "cloudigniter-system-admins": "system-admin",
...
const trustedSystemGroups = new Set(["system-admin", "system-super-admin"]);
...
export async function loadTenantSubject(input: {
  ...
  return ciCreateAuthorizationSubject(input.user, roleAssignments);
}
```
...
export async function resolveAuthorizationSubject(input: {
  ...
  return ciCreateAuthorizationSubject(input.user, [
    ...systemAssignments,
    ...scopedAssignments,
  ]);
}
