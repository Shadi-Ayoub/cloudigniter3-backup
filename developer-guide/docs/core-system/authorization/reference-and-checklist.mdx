...
This page summarizes the implemented `@cloudigniter/core` authorization surface. Import helpers from
`@cloudigniter/core/lib` and types from `@cloudigniter/core/types`.
...
Provider adapters and DynamoDB repositories may live in the AWS package or the application's server layer. The
catalog and decision engine remain in core-compatible code.
...
In `@cloudigniter/aws` or the application's server infrastructure. `@cloudigniter/core` remains provider-neutral.
