...
Use this reference to find CloudIgniter’s public authorization helpers and review your application’s access
controls before deployment. Import helpers from `@cloudigniter/core/lib` and types from `@cloudigniter/core/types`.
...
| `ciCanOverrideCoreAccessControl(subject, definition?)`     | Require a direct system-scoped `system-super-admin` assignment. |
...
for example `ciCreateAwsEmberguardProvider({ database, tables, keys })`, then use its member functions to load
...
- [ ] Only a directly assigned, system-scoped `system-super-admin` can create an override.
...
- [ ] The last active `system-super-admin` assignment cannot be removed.
