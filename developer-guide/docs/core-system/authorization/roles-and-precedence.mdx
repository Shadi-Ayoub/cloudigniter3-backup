---
title: 4. Roles, Inheritance, and Precedence
description: Build reusable groups, inherit privileges, and choose a deterministic conflict strategy.
sidebar_position: 4
---

# Roles, Inheritance, and Precedence

A role—also called a group—is a named set of privileges associated with a job, responsibility, or application
feature. Subjects receive access through scoped role assignments.

## Define focused roles

```ts
const roles = [
  {
    id: "invoice-viewer",
    title: "Invoice Viewer",
    precedence: 60,
    privileges: [
      {
        id: "read-invoices",
        title: "Read invoices",
        effect: "allow",
        resource: "billing.invoices",
        action: "read",
        scopeKinds: ["tenant", "orgUnit"],
      },
    ],
  },
  {
    id: "invoice-approver",
    title: "Invoice Approver",
    precedence: 30,
    inherits: ["invoice-viewer"],
    privileges: [
      {
        id: "approve-invoices",
        title: "Approve invoices",
        effect: "allow",
        resource: "billing.invoices",
        action: "approve",
        scopeKinds: ["tenant", "orgUnit"],
      },
    ],
  },
];
```

The approver inherits the viewer's privileges, avoiding duplication.

```mermaid
flowchart BT
  V[invoice-viewer] --> R[read invoices]
  A[invoice-approver] -->|inherits| V
  A --> P[approve invoices]
  M[billing-manager] -->|inherits| A
  M --> U[void invoices]
```

## Inheritance rules

- `inherits` contains role IDs from the same catalog.
- Inheritance may have several levels.
- A role may inherit more than one role.
- Cycles such as `a -> b -> a` are rejected by catalog validation.
- An inherited privilege is evaluated using the precedence of the role that was assigned to the subject.
- The decision evidence records both `assignedRoleId` and `privilegeRoleId`.

## Temporarily suspend a role

Use the role row menu on `/dashboard/security/roles` to suspend a role during an incident, investigation, or
temporary operational hold. The confirmation requires a reason and stores the actor, timestamp, and reason for
the latest status transition. The table keeps the role visible with a **Suspended** status so administrators can
inspect and restore it.

Suspension is reversible and does not delete assignments or policy configuration. While a role is suspended:

- its direct privileges grant no access;
- an assignment to it returns `suspended-role` when no other scoped grant applies;
- it contributes no inherited privileges and does not bridge an inheritance path;
- other active roles and direct subject privileges continue to evaluate normally.

Restoring the role makes its configured grants effective again through the existing assignments. Restoration
also requires a reason. There is no automatic expiry: an incident hold remains until an administrator restores
it. The `system-super-admin` break-glass role cannot be suspended, and changing any other core role requires a
directly assigned system super administrator.

Catalogs that omit `status` are treated as active. Server enforcement must authorize against the persisted
definition loaded from the provider; recreate a compiled authorizer whenever that definition changes.

## Manage direct roles and privileges in the dashboard

On `/dashboard/security/roles`, both the create and edit dialogs use searchable multi-select fields for direct
inheritance and privileges. Start typing a title, stable ID, resource, or action to reveal matching suggestions,
then select a result to add it as a chip. Use the chip's `X` button—or Backspace from an empty search field—to
remove a direct selection.

The stable identifier remains editable for the complete create session. After the role is saved, edit dialogs
show that identifier as read-only because assignments and audit evidence may already reference it. New
application role IDs must start with a lowercase letter and use lowercase kebab case, such as
`invoice-approver` or `reviewer2`. The form shows a field-level correction message and disables Save for spaces,
uppercase letters, underscores, leading digits, repeated hyphens, or other unsupported characters. Privilege
IDs follow the same rule.

The inheritance field displays direct parents only. It does not add chips for roles inherited transitively. For
example, selecting `invoice-approver` does not also add `invoice-viewer`, even though the approver inherits the
viewer. The effective policy still expands that relationship during authorization.

To prevent cycles, suggestions exclude:

- the role currently being edited;
- roles already selected as direct parents;
- any role that directly or transitively inherits the current role.

The privilege selector copies the selected privilege's complete policy statement into the role. Removing a
privilege chip removes that direct statement when the role is saved. Server-side catalog validation remains
authoritative if a stale client or concurrent change produces an invalid inheritance graph.

Example decision evidence for an approver using the inherited read permission:

```ts
decision.matches[0];
// {
//   source: "role",
//   assignedRoleId: "invoice-approver",
//   privilegeRoleId: "invoice-viewer",
//   precedence: 30,
//   privilege: { id: "read-invoices", ... },
//   assignmentScope: { kind: "tenant", tenantId: "tenant-123" }
// }
```

## Precedence

Lower numbers represent stronger precedence, matching CloudIgniter's existing Cognito-style convention.

```text
system-super-admin  0
system-admin       10
tenant-admin       20
approver           30
editor             40
viewer             50
```

Precedence is not scope. A system role assigned at a tenant remains a tenant-scoped assignment unless its
assignment says otherwise.

## Default combining algorithm: deny overrides

```ts
const authorizer = ciCreateAuthorizer(appAccessControl, {
  combiningAlgorithm: "deny-overrides",
});
```

This is the default and recommended algorithm. The authorizer evaluates every matching direct and role
privilege. If any applicable privilege says `deny`, the decision is denied.

```mermaid
flowchart TD
  A[Collect all matching privileges] --> B{Any deny?}
  B -->|yes| D[Deny]
  B -->|no| C{Any allow?}
  C -->|yes| E[Allow]
  C -->|no| D
```

This mode favors safe restrictions over group precedence. A lower-precedence restriction role can block an
operation otherwise granted by an administrator role.

## Optional algorithm: highest precedence

Use this algorithm only when the strongest applicable group should decide:

```ts
const authorizer = ciCreateAuthorizer(appAccessControl, {
  combiningAlgorithm: "highest-precedence",
});
```

The evaluator:

1. selects matching direct privileges if any exist;
2. otherwise selects matching privileges from the numerically lowest role-precedence tier;
3. applies deny-overrides within that selected tier.

```mermaid
flowchart TD
  A[Matching privileges] --> B{Direct match exists?}
  B -->|yes| C[Select direct tier]
  B -->|no| D[Select lowest role precedence number]
  C --> E{Deny in selected tier?}
  D --> E
  E -->|yes| F[Deny]
  E -->|no| G[Allow]
```

### Conflict example

Alice has two roles in the same tenant:

```text
editor, precedence 40: allow identity.users.update
blocked-editor, precedence 60: deny identity.users.update
```

| Algorithm            | Result  | Reason                                                  |
| -------------------- | ------- | ------------------------------------------------------- |
| `deny-overrides`     | Denied  | Every applicable deny wins.                             |
| `highest-precedence` | Allowed | `editor` has stronger precedence than `blocked-editor`. |

Choose the algorithm at application design time. Do not switch algorithms per request unless two clearly
separate policy domains require it.

## Explicit deny patterns

Explicit denies are useful for constraints that should remain visible in the policy:

```ts
{
  id: "prevent-user-deletion",
  title: "Prevent user deletion",
  effect: "deny",
  resource: "identity.users",
  action: "delete",
  scopeKinds: ["tenant", "orgUnit"],
}
```

Use them for cases such as:

- read-only or suspended operational roles;
- temporary incident restrictions;
- protecting destructive actions from broad wildcard roles;
- separating support access from security administration.

Default deny is still the primary restriction mechanism: if no privilege allows an action, it is denied. Do
not create explicit denies for every action a role lacks.

## Role engineering guidelines

### Name roles after responsibilities

Prefer:

```text
invoice-approver
support-agent
user-security-admin
report-viewer
```

Avoid roles based on individuals or temporary organizational labels:

```text
alice-access
team-7-special
old-admin
```

### Keep sensitive responsibilities separate

An application may intentionally separate:

- creating invoices from approving them;
- inviting users from assigning security roles;
- editing settings from rotating credentials;
- requesting refunds from executing refunds.

Assigning two focused roles is clearer than one role with every capability.

### Keep privilege IDs stable

Privilege IDs are used in decision evidence and persistence. Changing a description is harmless; renaming a
privilege should be treated as a policy migration.

### Avoid inheritance purely to reduce typing

Inheritance should express a real responsibility hierarchy. If two unrelated roles happen to share one
privilege, define that privilege in both roles or introduce a meaningful shared base role.

## Keep business and technical administration separate

CloudIgniter's core administrator roles form two inheritance lanes:

```mermaid
flowchart TD
  U[user]
  U --> D[developer]
  U --> A[admin]
  A --> SA[super-admin]
  U --> SYSA[system-admin]
  SYSA --> SYSSA[system-super-admin]
```

- `admin` and `super-admin` represent application-business administration at global, tenant, or Org Unit
  scopes.
- `system-admin` and `system-super-admin` represent technical platform administration and protected bootstrap
  duties.
- `system-admin` inherits only `user`; it does not inherit `admin` or `super-admin`.
- `system-super-admin` inherits `system-admin`. Its protected core-override capability still requires a direct,
  system-scoped assignment.

When one person needs both business and technical responsibilities, assign one role from each lane at the
appropriate scopes. Do not connect the lanes through custom inheritance. Precedence remains a separate
conflict-resolution mechanism and does not imply that one role inherits another.

## Existing CloudIgniter core-role helpers

The default access-control definition includes the canonical `user`, `developer`, `admin`, `super-admin`,
`system-admin`, and `system-super-admin` roles. CloudIgniter also exports their precedence map and the
primary-role resolver:

```ts title="Resolve a core role's precedence" showLineNumbers
import {
  CI_CORE_ROLE_PRECEDENCE,
  ciResolvePrimaryRole,
} from "@cloudigniter/core/lib";

ciResolvePrimaryRole(["user", "admin"], CI_CORE_ROLE_PRECEDENCE);
// "admin"
```

`ciResolvePrimaryRole()` is useful for display, diagnostics, and adapting identity-provider groups. It does not
replace the scoped authorizer because it does not evaluate resource, action, or scope.

Core roles and their privileges cannot be redefined by `ciCreateAppAccessControl()`. Create an application role
that inherits the appropriate core role. A deliberate core change must use the audited override workflow, which
is restricted to a directly assigned `system-super-admin`.

See [`ciIsAccessControlKebabIdentifier()`](/docs/api-reference/authorization/ci-is-access-control-kebab-identifier)
for the identifier contract.

See [`CI_DEFAULT_ACCESS_CONTROL_DEFINITION`](/docs/api-reference/authorization/default-access-control-definition)
for the built-in resources and inheritance chain.

Next: [Scopes and Assignments](./scopes-and-assignments).
