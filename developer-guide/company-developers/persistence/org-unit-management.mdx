...
## Development resolution probes

The template's `appGetOrgUnitLookupByPath` selects the built-in `CI_MOCK_ORG_UNITS` source only for the exact
reserved active probe tenant in `CI_DEV_TENANT_RESOLUTION_PROBES`, after `appGetDevBeaconAccess` verifies the
environment, authenticated actor, and enabled developer capability. All other requests retain the Amplify
`GetOrgUnitByPath` lookup. Missing candidates within the authorized probe tenant return `exists: false` without
querying the database. Probe fixtures include explicit predecessor IDs for nested nodes.

`packages/next` owns the internal HTTP transport: `ciLookupOrgUnit` forwards only the session cookie alongside
the JSON accept header to the same-origin lookup and refuses redirects. It does not forward page context as
authorization evidence. The endpoint repeats session authorization and returns uncached responses. This keeps
the [Dev Beacon](/dictionary/d#dev-beacon) report exercising the actual lookup and longest-prefix resolver after
application data moves to DynamoDB, without creating probe records in the System table.
