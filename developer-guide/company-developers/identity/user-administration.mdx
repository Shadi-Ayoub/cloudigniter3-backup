...
User create/edit dialogs consume compiled Core Smart Form definitions, with application extensions passed through
`src/custom/user` and stored under `profile.extensions`. Scoped assignment controls stay inside the shared form
as domain UI. See [Smart Form architecture](../architecture/smart-forms.mdx) for generation and cache boundaries.
...
### AWSJSON transport boundary

Core inputs and seeder fixtures keep `address`, `extensions`, `statusChange`, and `deletion` as structured values.
Amplify models all four as `a.json()`, which produces AppSync `AWSJSON`; AppSync requires the GraphQL variable to
contain a JSON string. Template server composition passes every User Profile mutation through
`ciSerializeUserProfileAwsJsonFields()` from `@cloudigniter/aws/lib`. That adapter omits `undefined`, preserves
`null` for restore, and serializes every other value exactly once. Reads use `ciDeserializeAwsJson()` before shape
guards and domain projection, accepting both encoded strings and values an Amplify client already decoded.

Do not stringify fixture data or change `CIUserPostalAddress` to a string. Validate both the synthesized scalar
type and the exact create/update variables: an unknown input field indicates deployment drift, while an invalid
value for an active `AWSJSON` field indicates wire-shape drift. Regression coverage must include the test-user
address and provenance extension, status transition metadata, deletion metadata, and restore `null` behavior.
...
and the invoked user operations retain their normal administration permissions. Profile-create failure triggers a
Cognito rollback; repeat seeding recognizes decoded provenance and skips an already-owned active account.
