...
- `apps/template/src/kernel/server/routes.ts`, which composes package-owned core routes with `src/custom/routes` and is imported directly by `cloudigniter.config.ts`;
...
| Area | V1 result |
| --- | --- |
| Routes | `apps/template/src/kernel/server/routes.ts` is a thin composition file; manual and generated custom routes live under `src/custom/routes`; strict merging rejects shadowing. |
