...
2. The **Amplify binding** in `apps/template/amplify` connects that contract to
   concrete Amplify function factories and Data models.

The separation keeps `@cloudigniter/aws` independent of an application template
and prevents Amplify construct details from leaking into reusable backend logic.
...
flowchart TD
  ...
  AmplifyFiles[Function factories and Data models in apps/template/amplify]
...
The package contract does not create a Lambda or table. The template supplies the
concrete Amplify definitions.
...
apps/template/amplify/functions/billing/create-invoice/
├── handler.ts
└── resource.ts
```
...
For an Amplify-managed DynamoDB table, define an `a.model(...)` in
`apps/template/amplify/data/schemata`. The manifest later resolves that model's
synthesized table construct.

## Layer 4: the Amplify manifest

`CI_CORE_AMPLIFY_MANIFEST` is the template-side binding point. It is organized by
feature, not by deployment stage. Its types, validation, and compiler are imported
from `@cloudigniter/aws/server/backend`; the application retains only values that
must reference its concrete Amplify factories and schema model names.
...
5. Create `handler.ts` and `resource.ts` in the matching template feature folder.
...
7. Add one `functions` binding to the feature in
   `CI_CORE_AMPLIFY_MANIFEST`.
...
5. Add the `a.model(...)` schema in the template Data schema.
