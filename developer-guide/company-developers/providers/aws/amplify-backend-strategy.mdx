...
resource factory. `ciDefineAmplifyBackendManifest(...)` inspects Amplify
function factories and rejects a declared group that differs from the concrete
...
owners, function-factory resource-group drift, empty/reserved/duplicate backend
keys, duplicate logical tables, duplicate Data model bindings, and duplicate
output names.
...
CloudIgniter's seven Cognito user-administration functions are AppSync resolvers,
so their factories and manifest feature use `resourceGroupName: "data"`. Their
Cognito IAM statements are generated by the post-build policy plan beneath each
Lambda, which keeps the policy in Data while referencing the User Pool in Auth.
Do not grant these resolvers through `defineAuth(...access)`: Amplify creates that
policy in Auth and attaches it to the Data execution role, recreating the reverse
edge even though the Lambda itself moved. Keep Cognito event triggers such as
post-confirmation in Auth.
...
directions before changing stack ownership or removing a dependency. For the
expected CloudIgniter graph, the Auth nested stack has no parameter sourced from
Data; Data may consume User Pool, identity-role, and Auth-trigger outputs.
